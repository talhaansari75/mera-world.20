...
export async function createRoom(userId:string,displayName:string,mode:string,maxPlayers=4){
  ...
  await getPrisma().$transaction(async tx=>{
    await tx.multiplayerRoom.create({data:{roomId:id,hostUserId:userId,mode:mode.slice(0,40) as DbGameMode,maxPlayers:Math.max(2,Math.min(8,maxPlayers)),members:{create:{userId,displayName:displayName.slice(0,40),role:"host"}}}});
  });
...
type RoomWithMembers = Prisma.MultiplayerRoomGetPayload<{ include: { members: { orderBy: { joinedAt: "asc" } } } }>;
