...
export const getAdminDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    return { ok: true as const, adminEmail: process.env.ADMIN_EMAIL?.trim() ?? "", summary: summary[0] ?? {}, players };
  });
...
export const trackPlayerActivity = createServerFn({ method: "POST" })
  ...
  .validator((d: { eventType?: string; screen?: string; durationSeconds?: number }) => ({
    ...
    durationSeconds: Math.max(0, Math.min(120, Math.floor(Number(d.durationSeconds ?? 0)))),
  }))
