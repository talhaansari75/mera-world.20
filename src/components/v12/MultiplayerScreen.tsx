...
export function MultiplayerScreen({ onBack }: { onBack: () => void }) {
  ...
  const refresh = async (id = room?.room_id || roomId) => { if (!id) return; setBusy(true); try { const r = await getMultiplayerRoom({ data: { roomId: id } }); if ((r as any).ok) { setRoom((r as any).room as Room); setRoomId(id); setMsg("Room is live. Share the room ID with your friends."); } else setMsg((r as any).error); } catch { setMsg("Unable to reach the multiplayer service."); } finally { setBusy(false); } };
  ...
  const create = async () => { setBusy(true); try { const r = await createMultiplayerRoom({ data: { displayName: name, mode: "classic", maxPlayers: 4 } }); if ((r as any).ok) { const next = (r as any).room as Room; setRoom(next); setRoomId(next.room_id); setMsg("Room created successfully."); } else setMsg((r as any).error); } catch { setMsg("Sign in to use online multiplayer."); } finally { setBusy(false); } };
  const join = async () => { setBusy(true); try { const r = await joinMultiplayerRoom({ data: { roomId: roomId.trim(), displayName: name } }); if ((r as any).ok) { setRoom((r as any).room as Room); setMsg("Joined room successfully."); } else setMsg((r as any).error); } catch { setMsg("Sign in to use online multiplayer."); } finally { setBusy(false); } };
