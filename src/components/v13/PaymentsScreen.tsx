...
export function PaymentsScreen({onBack}:{onBack:()=>void}) {
 ...
 return <Screen title="Purchases & Entitlements" onBack={onBack}>
  ...
  // @ts-ignore
  <div className="mt-3 flex flex-col gap-2">{rows.map(r=><div key={r.productId} className="panel flex justify-between rounded-xl p-3"><span className="text-fg">{r.productId}</span><span className="text-muted">{r.active ? "Active" : "Inactive"}</span></div>)}{!rows.length&&!error&&<p className="text-sm text-muted">No active entitlements.</p>}{error&&<p className="text-sm text-muted">{error}</p>}</div>
 </Screen>
}
