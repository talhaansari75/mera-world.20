...
export const Route=createFileRoute("/api/payments/refund")({server:{handlers:{POST:async({request})=>{try{
 const userId=await requireUserId(); const limited=await consumeRateLimit(userId,"payment_refund",3,3600); if(!limited.allowed)return json({error:"refund_rate_limited"},429); const body=await request.json() as {intentId?:string;reason?:string}; if(!body.intentId||!body.reason)return json({error:"intentId and reason are required"},400);
