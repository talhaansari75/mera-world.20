...
export const Route=createFileRoute("/api/multiplayer/action")({server:{handlers:{POST:async({request})=>{try{
 const userId=await requireUserId(),b=await request.json().catch(()=>({})) as Record<string,unknown>,db=getPrisma();
 ...
 if(action==="heartbeat"){await db.$queryRaw`update multiplayer_members set last_seen_at=now(),disconnected_at=null where room_id=${roomId} and user_id=${userId} and is_bot=false`;return json({ok:true});}
