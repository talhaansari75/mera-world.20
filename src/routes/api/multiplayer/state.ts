...
await tickBots(room[0].matchId);
const members=await db.$queryRaw`select display_name as "displayName",country_code as "countryCode",is_bot as "isBot",avatar_id as "avatarId",score,progress,combo,ready,last_seen_at as "lastSeenAt",bot_skill as "botSkill",found_words as "foundWords" from multiplayer_members where room_id=${roomId} order by score desc`;
...
const events=await db.$queryRaw`select seq,user_id as "userId",event_type as "eventType",payload,created_at as "createdAt" from multiplayer_match_events where match_id=${room[0].matchId} order by seq desc limit 100`; return json({room:room[0],members,messages:messages.reverse(),events:events.reverse()});
