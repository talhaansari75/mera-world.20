...
export const Route=createFileRoute("/api/multiplayer/match")({server:{handlers:{POST:async({request})=>{try{
 const userId=await requireUserId(),b=await request.json().catch(()=>({})) as Record<string,unknown>;
 ...
 await db.$transaction(async(tx:any)=>{
  ...
  await tx.$queryRaw`insert into multiplayer_members(room_id,user_id,display_name,role,country_code,is_bot,avatar_id,ready,last_seen_at) values(${roomId},${userId},'Traveler','player',${cc},false,'traveler',false,now())`;
