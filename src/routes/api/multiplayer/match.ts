...
const userId=await requireUserId(); const b=await request.json().catch(()=>({})) as {countryCode?:string;maxPlayers?:number;mode?:string;worldId?:number;durationSeconds?:number};
...
const duration=Math.min(600,Math.max(30,Number(b.durationSeconds||(mode==="blitz"?90:180)))),seed=Math.floor(Math.random()*2147483647);
...
await tx.$queryRaw`insert into multiplayer_matches(match_id,room_id,status,mode,puzzle_seed,world_id,duration_seconds) values(${matchId},${roomId},'waiting',${mode},${seed},${Number(b.worldId||1)},${duration})`;
