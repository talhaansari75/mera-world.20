...
import { C as targetTimeMs, S as specFor, a as HINT_COST, h as modeMods, i as ENERGY_REFILL_MS, m as isWord, p as isBoss, v as puzzleForDaily, w as todayKey, y as puzzleForLevel } from "./levels-DBG2bxfj.mjs";
import { B as rewardCoins, C as modeRules, E as observeCompletion, H as specialKindsAt, J as xpForClear$1, M as petProfile, O as personalizedRewardMultiplier, P as petXpEarned, S as migrateSave, U as specialTilesForPuzzle, W as starsFor, _ as isJourneyBoss, c as clipAtLocked, d as dailyChallengeRewardMultiplier, f as defaultSave, g as intelligenceAdaptivePlan, j as petPower, k as petEffect, l as dailyChallengeFor, u as dailyChallengeObjective, v as journeyWorldForLevel } from "./rewardPersonalization-BDFGfk_j.mjs";
import { randomUUID } from "node:crypto";
//#region node_modules/.nitro/vite/services/ssr/assets/gameplay-BF0n38Gn.js
...
var recordValue = (v) => v && typeof v === "object" && !Array.isArray(v) ? v : {};
...
var startGameplaySession = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((d) => ({
	...
	try {
		...
		const save = row?.saveJson ? migrateSave(JSON.parse(row.saveJson)) : defaultSave();
		...
		if (!admin && data.kind === "level" && !rules.free && energy < energyCost) return {
			ok: false,
		...
		try {
			await db.$transaction(async (tx) => {
				...
				const currentSave = currentRow?.saveJson ? migrateSave(JSON.parse(currentRow.saveJson)) : defaultSave();
			...
			return {
				ok: false,
				error: `GAMEPLAY_START_ERROR: ${(error instanceof Error ? error.message : String(error)).slice(0, 300)}`
			};
		}
...
var recordGameplayAction = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((d) => {
	...
	return getPrisma().$transaction(async (tx) => {
		...
		if (data.type === "hint") {
			...
			const equippedPet = typeof save.equippedPet === "string" ? save.equippedPet : null;
			Number(recordValue(save.petLevels)[equippedPet ?? ""] ?? 1);
...
var startBossSession = createServerFn({ method: "POST" }).middleware([authMiddleware]).validator((d) => ({ level: Math.max(1, Math.min(2e3, Math.floor(Number(d.level) || 1))) })).handler(startBossSession_createServerFn_handler, async ({ context, data }) => {
	...
	return db.$transaction(async (tx) => {
		...
		const gear = Array.isArray(save.equipment) ? save.equipment.reduce((n, e) => n + (recordValue(save.equippedEquipment)[e.slot] === e.id ? Number(e.power || 0) : 0), 0) : 0;
